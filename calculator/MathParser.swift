...
func evaluateExpression(expression: String, angleUnit: MathParserAngleUnit) -> Double? {
    ...
    mainLoop: for index in expression.characters.indices {
        let character = expression[index]
        
        if (character == "-" && index == expression.startIndex) || (character == "-" && expression[index] == "(") {
        ...
        if lastWasNumber && character.isNumber {
            ...
            case "(": //Each time a bracket is found buffer is force parsed
                if !buffer.isEmpty {
                    if mathParserConstants.numberOfKeysStartingWith(searchString: buffer)! > 0 {
                        if let constant = mathParserConstants[buffer] {
                            numbers.append(constant.value)
                        }
                    } else if mathParserOperators.numberOfKeysStartingWith(searchString: buffer)! > 0 {
            ...
            if mathParserConstants.numberOfKeysStartingWith(searchString: buffer) == 1 {
                if let constant = mathParserConstants[buffer] {
                    numbers.append(constant.value)
                    lastWasNumber = true
                    buffer = ""
                }
            } else if mathParserOperators.numberOfKeysStartingWith(searchString: buffer) == 1 {
                if mathParserOperators[buffer] != nil {
                    var operation =  mathParserOperators[buffer]!
                    operation.priority += normalPriority
    ...
    if !buffer.isEmpty {
        ...
        } else if mathParserConstants.numberOfKeysStartingWith(searchString: buffer)! > 0 {
            if let constant = mathParserConstants[buffer] {
                numbers.append(constant.value)
            }
        } else if mathParserOperators.numberOfKeysStartingWith(searchString: buffer)! > 0 {
    ...
    while operations.count > 0 {
        ...
        numbers[startIndex] = operation.evaluate(numbers: numbersToEvaluate)
